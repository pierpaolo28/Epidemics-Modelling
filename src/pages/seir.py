...
def write():
    with st.spinner("Loading Advanced SEIR Modelling ..."):
        ...
        st.markdown(
            '''
            In this section, is taken a step further the SEIR model proposed in the "SIR and SEIR Modelling" tab.
            In this improved version, we now: 
            - Take into account the portion of infected individuals which dies instead of recover.
            - $R_{0}$ is not anymore static but dynamically changes over time. In this example, two functions have been used 
            in order to simulate $R_{0}$ behaviour over time: a Sigmoid or Sinusoidal. In this way, we are now able to model
            how a government might react in order to control the spread of the desease by exercising social distancing measures.
            A sigmoid in it's minimum point can in fact represent a lockdown and the smoothness by which it reaches its minimum can 
            easily represent how gradually the restrictions have been applied. An additionall parameter is provided in order to decide from what day onwards 
            to start applying the restrictions (so that to observe what could be the consequences of a late or early intervention). The sinusoidal 
            landscape, can instead be used in order to model possible sequential waves a desease can lead to.
            -  Also the death rate has been designed to be time and age dependent. To each different age group is assigned a different base death rate (the older, the greater), which increase linearly
            with the increase in the number of infected at each time-step. Therefore, with higher peaks of individuals infected all at the same time
            increases the likelihood of individuals to die (mimicking strained healthcare system which don't have the potential to cure everyone at the same time). 
            An animation of how the percentage of people dying if positive ($\\alpha$) varies over time, is available in the animated simualation below.
            '''
        )
        ...
        if (sum(age_props) == 1) or len(age_props) == 4:
            ...
            if land == 'Sigmoid':
                ...
                dec_factor = st.slider("How drasticly rapidly have the restrictions been applied?",
                                       ...
                                       value=0.3, step=0.1)
                R0_beg = st.number_input("Maxium possible R value", 1, 14, 6)
